...
try {
    Invoke-Step 'Check clustering tools and cluster object' {
        if (-not (Get-Command Get-Cluster -ErrorAction SilentlyContinue)) {
            throw 'Failover Clustering tools are not installed on this management host.'
        }
    ...
    Invoke-Step 'Verify cluster node membership and state' {
        ...
        if ($expected -ne $actual) {
            throw "Node mismatch. Expected: $expected; actual: $actual"
        }
    ...
    Invoke-Step 'Verify Storage Spaces Direct health' {
        ...
        if (Get-Command Get-StoragePool -ErrorAction SilentlyContinue) {
            Get-StoragePool -IsPrimordial $false |
                Format-Table FriendlyName, HealthStatus, OperationalStatus | Out-Host
        }
        if (Get-Command Get-VirtualDisk -ErrorAction SilentlyContinue) {
            Get-VirtualDisk |
                Format-Table FriendlyName, HealthStatus, OperationalStatus, ResiliencySettingName | Out-Host
        }
    }
