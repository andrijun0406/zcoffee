...
function Invoke-AzureCli {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Azure CLI failed:`n$($output -join [Environment]::NewLine)"
    }
    return $output
}
...
try {
    ...
    Invoke-Step 'Confirm Azure context' {
        ...
        if ($TenantId -and $account.tenantId -ne $TenantId) {
            throw "Tenant mismatch. Expected $TenantId; current $($account.tenantId)."
        }
    ...
    if (-not $EnableDeployment) {
        throw 'Deploy mode is disabled by default. Re-run with -EnableDeployment after review.'
    }

    Invoke-Step 'Preview changes with what-if' {
        Invoke-AzureCli -Arguments @(
